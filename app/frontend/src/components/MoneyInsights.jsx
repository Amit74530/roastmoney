...
export default function MoneyInsights({ transactions, previousTransactions }) {
  ...
  return (
    <section className="money-report card">
      <div className="section-head">
        <div>
          <span className="eyebrow">Behaviour analysis</span>
          <h2>The money report</h2>
        </div>
        <Lightbulb size={19} />
      </div>
      <div className="insight-list">
        {insights.map((insight, index) => (
          <div className="report-insight" key={insight}>
            <span>0{index + 1}</span>
            <p className="roast-quote">{insight}</p>
            <ArrowRight size={15} />
          </div>
        ))}
      </div>

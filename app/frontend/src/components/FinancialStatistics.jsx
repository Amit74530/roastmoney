...
export default function FinancialStatistics({ transactions }) {
  ...
  const cards = [['HIGHEST EXPENSE', stats.highestExpense ? money(stats.highestExpense.amount) : '₹0', stats.highestExpense?.merchant || 'No expense recorded', TrendingDown], ['AVERAGE TRANSACTION', money(stats.averageTransaction), 'Expense average', BarChart3], ['MOST USED CATEGORY', stats.mostUsedCategory, 'By expense total', Receipt], ['TOTAL TRANSACTIONS', stats.totalTransactions, 'In this period', Hash]]

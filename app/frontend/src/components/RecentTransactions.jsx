...
export default function RecentTransactions({ transactions, onAdd }) {
  ...
  return <section className="card recent"><div className="section-head"><div><span className="eyebrow">LIVE LEDGER / LATEST 5</span><h2>RECENT TRANSACTIONS</h2></div><Link to="/transactions" className="text-link">VIEW ALL TRANSACTIONS <ArrowRight size={15} /></Link></div><div className="tx-list">{recent.map((item) => { const income = transactionIsIncome(item); return <div className="tx-row" key={item.id}><div className="merchant-icon">{iconFor(item.category)}</div><div className="tx-name"><strong>{item.title}</strong><small>{item.category} · {item.transaction_date}</small></div><span className={`money-direction ${income ? 'income' : 'expense'}`} title={income ? 'Income' : 'Expense'}>{income ? <ArrowDownLeft size={15} /> : <ArrowUpRight size={15} />}</span><strong className={`tx-amount ${income ? 'income-text' : ''}`}>{income ? '+' : '-'}{money(transactionAmount(item))}</strong></div> })}</div></section>
}

...
export default function FinancialSummary({ transactions }) {
  ...
  const cards = [
    ['TOTAL BALANCE', summary.totalBalance, 'INCOME - EXPENSES', CircleDollarSign, summary.totalBalance >= 0 ? 'lime-text' : 'red-text'],
    ['TOTAL INCOME', summary.totalIncome, 'MONEY IN', ArrowDownLeft, 'lime-text'],
    ['TOTAL EXPENSES', summary.totalExpenses, 'MONEY OUT', ArrowUpRight, 'orange-text'],
    ['TOTAL SAVINGS', summary.totalSavings, 'CURRENT BALANCE', PiggyBank, summary.totalSavings >= 0 ? 'lime-text' : 'red-text'],
  ]
